                 ...
                 const double y2 = 0.84)
{
  ...
  legend.AddEntry(histograms.bdt.get(), "NO_SPLIT BDT", "l");
  legend.AddEntry(histograms.mlp.get(), "NO_SPLIT MLP", "l");
  legend.DrawClone();
}
...
void draw_information_panel(HistogramPair& histograms, const char* binning_text)
{
  TLatex label;
  label.SetNDC();
  label.SetTextAlign(13);
  ...
  label.DrawLatex(0.20, 0.73, "Common valid clusters");
  label.DrawLatex(0.20, 0.64, binning_text);
  draw_legend(histograms, 0.20, 0.36, 0.88, 0.55);
}
    ...
    const std::string output_base = "PhotonAnalysisTree/output/plots/nosplit_score_comparison")
{
  ...
  HistogramPair inclusive = make_histogram_pair("inclusive");
  ...
  {
    truth_pt_histograms.push_back(make_histogram_pair("truth_pt_" + std::to_string(bin)));
  }
  ...
  {
    cluster_et_histograms.push_back(make_histogram_pair("cluster_et_" + std::to_string(bin)));
  }
  ...
  TCanvas inclusive_canvas("c_score_inclusive", "Inclusive score comparison", 1000, 800);
  draw_histograms(inclusive);
  draw_legend(inclusive, 0.55, 0.55, 0.90, 0.72);
  draw_sphenix_label("Single #pi^{0} gun, common valid clusters");
  inclusive_canvas.RedrawAxis();
  inclusive_canvas.SaveAs((output_base + "_inclusive.pdf").c_str());

  TCanvas truth_pt_canvas("c_score_truth_pt", "Score comparison by truth pi0 pT",
                          1500, 900);
  ...
  draw_information_panel(truth_pt_histograms.front(), "Binned in truth #pi^{0} p_{T}");
  truth_pt_canvas.SaveAs((output_base + "_truth_pt.pdf").c_str());

  TCanvas cluster_et_canvas("c_score_cluster_et", "Score comparison by cluster ET",
                            1500, 1200);
  ...
  draw_information_panel(cluster_et_histograms.front(), "Binned in NO_SPLIT cluster E_{T}");
  cluster_et_canvas.SaveAs((output_base + "_cluster_et.pdf").c_str());

  std::cout << "PlotNosplitScoreComparison - events/total clusters/common valid clusters/"
