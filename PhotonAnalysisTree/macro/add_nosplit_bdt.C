...
int add_nosplit_bdt(
    ...
    const char* model_path =
        "/sphenix/user/shuhangli/ppg12/FunWithxgboost/binned_models/model_base_v3E_nosplit_single_tmva.root")
{
  constexpr const char* tree_name = "event_tree";
  constexpr const char* score_branch = "nosplit_cluster_bdt_base_v3E_score";
  constexpr const char* valid_branch = "nosplit_cluster_bdt_base_v3E_valid";
  ...
  {
    std::cerr << "NO_SPLIT BDT score or valid branch already exists" << std::endl;
    return 4;
  }
  ...
  ok &= bind(tree, "nosplit_cluster_et", &cluster_et);
  ok &= bind(tree, "nosplit_cluster_eta", &cluster_eta);
  ok &= bind(tree, "nosplit_cluster_shower_valid", &shower_valid);
  ok &= bind(tree, "nosplit_cluster_shower_w_eta_cogx", &weta);
  ok &= bind(tree, "nosplit_cluster_shower_w_phi_cogx", &wphi);
  ok &= bind(tree, "nosplit_cluster_shower_e11_over_e33", &e11_over_e33);
  ok &= bind(tree, "nosplit_cluster_shower_e32_over_e35", &e32_over_e35);
  ok &= bind(tree, "nosplit_cluster_shower_et1", &et1);
  ok &= bind(tree, "nosplit_cluster_shower_et2", &et2);
  ok &= bind(tree, "nosplit_cluster_shower_et3", &et3);
  ok &= bind(tree, "nosplit_cluster_shower_et4", &et4);
  if (!ok)
  {
    return 5;
  }
  ...
  {
    std::cerr << "Cannot create NO_SPLIT BDT output branches" << std::endl;
    return 7;
  }
  ...
  for (Long64_t entry = 0; entry < tree->GetEntries(); ++entry)
  {
    ...
    {
      std::cerr << "Failed to fill NO_SPLIT BDT output branches at entry " << entry << std::endl;
      return 9;
    }
  }
  ...
  {
    std::cerr << "NO_SPLIT BDT output branch entry count mismatch" << std::endl;
    return 9;
  }
  file->cd();
  ...
  std::cout << "add_nosplit_bdt - events/clusters/valid/malformed = "
