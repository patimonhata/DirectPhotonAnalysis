        ...
        "model_ppg15_nominal_base_v3E_split_3to35_allplus3jet40_ppg12split_single_tmva.root")
{
  ...
  constexpr const char* score_branch = "split_cluster_bdt_ppg15v1_score";
  constexpr const char* valid_branch = "split_cluster_bdt_ppg15v1_valid";
  ...
  {
    std::cerr << "PPG15v1 SPLIT BDT score or valid branch already exists" << std::endl;
    return 4;
  }
  ...
  {
    std::cerr << "Cannot create PPG15v1 SPLIT BDT output branches" << std::endl;
    return 7;
  }
  ...
  for (Long64_t entry = 0; entry < tree->GetEntries(); ++entry)
  {
    ...
    {
      std::cerr << "Failed to fill PPG15v1 SPLIT BDT output branches at entry " << entry << std::endl;
      return 9;
    }
  }
  ...
  {
    std::cerr << "PPG15v1 SPLIT BDT output branch entry count mismatch" << std::endl;
    return 9;
  }
  file->cd();
  ...
  std::cout << "add_split_bdt_ppg15v1 - events/clusters/valid/malformed = "
