...
int check_pythia_truth_pt_partial(const std::string input_file)
{
  ...
  const bool valid_metadata = schema_version == 1 && photon_selection &&
      *photon_selection == "prompt_category_1_or_2" && manifest_path &&
      ...
      bin_width_normalized == 0U && malformed_event_count == 0ULL &&
      invalid_weight_event_count == 0ULL;
  if (!valid_metadata)
  {
  ...
  std::cout << "check_pythia_truth_pt_partial - range/files/events/prompt/pi0/pi0 decay = ["
            << manifest_begin << ":" << manifest_end << "]/" << files_added << "/"
            << events_processed << "/" << prompt_photon_count << "/" << pi0_count
            << "/" << pi0_decay_photon_count << std::endl;
  return 0;
}
