...
split_bdt_output=${2:?usage: run_add_scores.sh INPUT BDT_OUTPUT FINAL_OUTPUT [BDT_MODEL] [ONNX_MODEL]}
both_bdt_output=${3:?usage: run_add_scores.sh INPUT BDT_OUTPUT FINAL_OUTPUT [BDT_MODEL] [ONNX_MODEL]}
final_output=${4:?usage: run_add_scores.sh INPUT BDT_OUTPUT FINAL_OUTPUT [BDT_MODEL] [ONNX_MODEL]}
split_bdt_model=${5:-/sphenix/user/shuhangli/ppg12/FunWithxgboost/binned_models/model_base_v3E_split_single_tmva.root}
nosplit_bdt_model=${6:-/sphenix/user/shuhangli/ppg12/FunWithxgboost/binned_models/model_base_v3E_split_single_tmva.root}
onnx_model=${7:-$module_dir/models/best_model.onnx}
...
root -l -b -q \
  ...
  -e "gSystem->Exit(add_nosplit_gamma_onnx(\"$both_bdt_output\",\"$final_output\",\"$onnx_model\"));"
root -l -b -q \
