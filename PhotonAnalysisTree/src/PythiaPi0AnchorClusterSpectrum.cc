...
int PythiaPi0AnchorClusterSpectrum::process_event(PHCompositeNode* topNode)
{
  ...
  for (const photon_tree::Pi0TopologyAnchorRecord& anchor : result.anchors)
  {
    ...
    case photon_tree::Pi0AnchorTopology::merged:
      ...
      ++n_merged_;
      break;
    case photon_tree::Pi0AnchorTopology::single_contaminated:
      h_single_contaminated_->Fill(et);
      ++n_single_contaminated_;
      break;
...
void PythiaPi0AnchorClusterSpectrum::create_output()
{
  ...
  h_merged_ = new TH1D(
      ...
      n_bins_, 0.0, et_max_);
  h_single_contaminated_ = new TH1D(
      "h_pi0_anchor_single_contaminated_cluster_et_raw", "", n_bins_, 0.0, et_max_);
  h_missing_ = new TH1D(
