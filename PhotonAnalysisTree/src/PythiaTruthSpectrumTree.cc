...
int PythiaTruthSpectrumTree::process_event(PHCompositeNode* topNode) {
  ...
  b_event_uid_ = (static_cast<unsigned long long>(source_file_id_) << 32U) | static_cast<unsigned long long>(b_event_in_file_);
  ++n_events_processed_;
  ...
  event_tree_->Fill();
  ++n_events_written_;
  /* print info only for the first 5 events */
  if (verbosity_ > 0 && b_event_in_file_ < 5U) {
    std::cout << "PythiaTruthSpectrumTree::process_event - event " << b_event_in_file_
              << " Particles/FinalPhotons/TerminalPi0 = " << b_hepmc_n_particle_record_ << "/" << b_truth_photon_n_ << "/" << b_truth_pi0_n_ << std::endl;
  }
  return Fun4AllReturnCodes::EVENT_OK;
}
...
bool PythiaTruthSpectrumTree::fill_truth(const PHHepMCGenEventMap* event_map, const PHG4TruthInfoContainer* truth) {
  ...
  if (!signal_event || !event || !signal_event->is_simulated() || !truth) {
    return false; /* , which means invalid event */
  }
  ...
  for (auto iterator = event->particles_begin(); iterator != event->particles_end(); ++iterator) {
    ...
    if (particle->pdg_id() == 22 && particle->status() == 1 && !particle->end_vertex()) {
      ...
      b_truth_photon_immediate_parent_count_.push_back(static_cast<int>(immediate_parents.size()));
  ...
  for (auto iterator = secondary_range.first; iterator != secondary_range.second; ++iterator) {
    ...
    if (parent_embedding_id != signal_embedding_id_ || !hepmc_parent || hepmc_parent->pdg_id() != 111) {
