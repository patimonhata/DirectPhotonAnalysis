...
int Fun4All_PythiaClusterEtSpectra(
    ...
    const double response_max = 1.5)
{
  if (manifest_path.empty() || output_file.empty() || manifest_begin < 0 ||
      manifest_end <= manifest_begin)
  {
    std::cerr << "Fun4All_PythiaClusterEtSpectra - invalid argument" << std::endl;
    return EXIT_FAILURE;
  }
  std::ifstream manifest(manifest_path);
  if (!manifest)
  {
    std::cerr << "Fun4All_PythiaClusterEtSpectra - cannot open manifest: "
  ...
  while (std::getline(manifest, line) && row < manifest_end)
  {
    ...
    if (row >= manifest_begin)
    {
      ...
      {
        std::cerr << "Fun4All_PythiaClusterEtSpectra - invalid or duplicate suffix at row "
                  << row << ": " << line << std::endl;
        return EXIT_FAILURE;
      }
  ...
  {
    std::cerr << "Fun4All_PythiaClusterEtSpectra - manifest range is incomplete"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "Fun4All_PythiaClusterEtSpectra - range/files/output = ["
  ...
  {
    std::cerr << "Fun4All_PythiaClusterEtSpectra - failed (run=" << run_status
