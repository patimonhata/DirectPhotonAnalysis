...
if (( $# != 8 )); then
  echo "Usage: $0 FAMILY MAP_ROOT OUTPUT_BASE REQUIRE_COMPLETE TAGGING_PARTNER_MIN_ENERGY_GEV SAMPLE_NAME SHARD_INDEX SHARD_COUNT" >&2
  exit 2
fi
...
set -u
root -l -b -q -e ".L $workflow_dir/PlotDisplacedPartnerMassDiagnostic.C" \
  -e "gSystem->Exit(PlotDisplacedPartnerMassDiagnostic(\"$family\",\"$map_root\",\"$output_base\",$require_complete,$tagging_threshold,\"$sample_name\",0,$shard_index,$shard_count,false))"
