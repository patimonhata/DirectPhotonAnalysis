    ...
    const std::string selection)
{
  ...
  if (samples.empty() || partial_root.empty() || composition_output_base.empty() || topology_output_base.empty() || !find_selection(selection)) return 1;
  ...
  if (!make_output_directory(composition_output_base)) return 2;
  ...
  for (const SampleDefinition& sample : samples)
  {
    ...
    for (int shard_index = 0; shard_index < shard_count; ++shard_index)
    {
      ...
      if (file.IsZombie() || !read_metadata(file, metadata) || !read_shard_metadata(file, shard) || !read_topology_summary(file, topology_summary))
      {
        std::cerr << "Missing or invalid composition partial: " << path << std::endl;
        return 3;
      }
      if (metadata.schema_version != 3 || metadata.source_map_schema_version != 4 || metadata.family != family ||
          ...
          metadata.region_a_anchor_clusters > metadata.region_a_clusters || metadata.selected_cluster_count != metadata.prompt_cluster_count + metadata.pi0_cluster_count +
      ...
      for (std::size_t index = 0; index < category_count; ++index)
      {
        ...
        TH1D* count = file.Get<TH1D>(("composition/" + count_name).c_str());
        TH1D* weighted_pb = file.Get<TH1D>(("composition/" + weighted_name).c_str());
  ...
  for (std::size_t topology_selection = 0; topology_selection < kSelectionCount; ++topology_selection)
    if (!topology_histograms[topology_selection] || !::valid_partition(topology_histograms[topology_selection]->counts) ||
        !::valid_partition(topology_histograms[topology_selection]->weighted_pb)) return 6;
  ...
  draw_stack(fractions, composition_output_base + "_category_fraction_stack.pdf", family, selection, combined.min_cluster_energy, false);
  draw_stack(fractions, composition_output_base + "_category_fraction_stack_detailed.pdf", family, selection, combined.min_cluster_energy, true);

  TFile output((composition_output_base + ".root").c_str(), "RECREATE");
  ...
  std::vector<std::string> topology_selection_definitions = {
      "pass_kinematics",
  ...
  std::cout << "MergePythiaPhotonCandidateSelection - family/selection/samples/selected/output = "
            << family << "/" << selection << "/" << combined.sample_names.size() << "/" << combined.selected_cluster_count << "/" << composition_output_base << "/" << topology_output_base << std::endl;
