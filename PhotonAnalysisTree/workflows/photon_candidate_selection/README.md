...
Each selection also contains `workinprogress/region_a_candidate_truth_origin.pdf` plus work-in-progress copies of the summary composition survival and both summary and detailed category-fraction-stack plots. These slide-oriented plots use a 0--40 GeV x-axis, a larger plot pad, and a larger, compact caption without the stored/anchor threshold; its candidate line shows the kinematic range at the kinematic stage and only the applied cut at later stages. The origin and survival legends are inside the plot frame. Both summary and detailed work-in-progress category stacks place the caption and plot in the left column and the legend in a full-height right column. Stack legends are ordered from the top of the stack to the bottom. The origin plot compares the weighted prompt-photon-, pi0-, eta-, and other-candidate spectra after that selection; the five pi0 topology subclasses are summed into one pi0 curve. The displayed composition survival plots omit the redundant all-candidate curve, while its histogram remains available in `candidate_composition.root`.
