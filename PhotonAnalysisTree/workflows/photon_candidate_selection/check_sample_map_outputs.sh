...
usage="usage: workflows/photon_candidate_selection/check_sample_map_outputs.sh SAMPLE_NAME [FILES_PER_JOB] [DEEP_VALIDATION] [OUTPUT_DIRECTORY] [MIN_CLUSTER_ENERGY_GEV]"
sample_name=${1:?$usage}
files_per_job=${2:-10}
...
for ((chunk = 0; chunk < expected_maps; ++chunk)); do
  ...
  if [[ "$deep_validation" == true ]] && ! root -l -b -q "$workflow_dir/check_pythia_photon_candidate_map.C(\"$path\",${min_cluster_energy})"; then
    echo "INVALID $path"
    ((invalid += 1))
  fi
done
...
echo "check_sample_map_outputs - sample/min-cluster-E/files/files_per_map/expected/missing/invalid = " \
  "$sample_name/$min_cluster_energy/$total_files/$files_per_job/$expected_maps/$missing/$invalid"
