...
for ((chunk = 0; chunk < expected_maps; ++chunk)); do
  ...
  if [[ "$deep_validation" == true ]] && ! root -l -b -q "$workflow_dir/check_pythia_photon_candidate_map.C(\"$path\")"; then
    echo "INVALID $path"
    ((invalid += 1))
  fi
done
...
echo "check_sample_map_outputs - sample/files/files_per_map/expected/missing/invalid = $sample_name/$total_files/$files_per_job/$expected_maps/$missing/$invalid"
