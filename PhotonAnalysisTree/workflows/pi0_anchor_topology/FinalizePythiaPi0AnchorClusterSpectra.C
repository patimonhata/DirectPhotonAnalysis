...
bool read_metadata(const std::string& path, PartialMetadata& value)
{
  ...
  ok &= bind(tree, "photon_recovery_policy",
  ...
  ok &= bind(tree, "min_photon_energy_recovery",
             &value.min_photon_energy_recovery);
                ...
                const PartialMetadata& reference)
{
  return valid_metadata(value) &&
      ...
      value.photon_recovery_policy ==
          reference.photon_recovery_policy &&
      ...
      same_double(value.min_photon_energy_recovery,
                  reference.min_photon_energy_recovery);
}
...
int FinalizePythiaPi0AnchorClusterSpectra(
    const std::string partial_pattern =
        "output/pi0_anchor_topology_partial/eta07_full_partner_fgamma0p0_recovery0p5_clusterenergy/partial_*.root",
    const std::string output_base =
        "output/plots/pi0_anchor_topology/minimum_bias/eta07_full_partner_fgamma0p0_recovery0p5_clusterenergy",
    ...
    const long long expected_manifest_end = -1)
{
  ...
  recovery_label
      << "E_{clus} f_{dep}^{#gamma}/E_{truth}^{#gamma} #geq "
                 << reference.min_photon_energy_recovery;
  spectrum_label.DrawLatex(
      0.20, 0.52, recovery_label.str().c_str());
  ...
  fraction_label.DrawLatex(
      0.20, 0.68, recovery_label.str().c_str());
  ...
  metadata.Branch(
      "photon_recovery_policy", &total.photon_recovery_policy);
  ...
  metadata.Branch(
      ...
      &total.min_photon_energy_recovery);
  metadata.Branch(
