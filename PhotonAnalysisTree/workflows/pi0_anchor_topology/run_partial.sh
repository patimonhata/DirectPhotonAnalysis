...
min_photon_energy_recovery=${15:-0.0}
max_abs_vertex_z=${16:-60.0}
cemc_acceptance_eta_max=${17:-1.1}
min_direct_match_cluster_energy_coverage=${18:-0.5}
...
if [[ "$enable_missing_diagnostics" != true && "$enable_missing_diagnostics" != false ]]; then
  echo "ENABLE_MISSING_DIAGNOSTICS must be true or false" >&2
  exit 2
fi
