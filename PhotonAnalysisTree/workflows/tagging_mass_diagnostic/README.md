...
Mass uses 240 bins from 0 to 1.2 GeV (5 MeV per bin). The candidate ET axis spans 0–40 GeV. Shape overlays use only 5–6, 6–8, 8–10, 10–18 and 18–30 GeV, identified by the candidate-cluster ET legend. The saved 2D histograms additionally retain 0–5 and 30–40 GeV bins, without adding these intervals to the shape overlay. Raw and weighted shape captions explicitly state normalization within each candidate ET bin. Mass overflow is retained and included in shape-normalization denominators; only 0–1.2 GeV is displayed. An ET bin with zero signed normalization is displayed as an empty shape. These fixed ET bins define the saved 2D axes.
...
Diagnostic output schema is now 2 for the revised ET axes. Re-run reduce from maps and merge into a new output directory; schema-1 partials cannot recover the new 18 GeV boundary from their coarse ET bins and are rejected. Map regeneration is not needed for this plotting/binning update.
